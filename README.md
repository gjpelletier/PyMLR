# Python Tools for Machine Learning Regression and Classification
...
- Building predictive models for continuous numerical or categorical data.
- Automating hyperparameter tuning for better model performance.
- Explaining model predictions using SHAP values and feature importance.
...
The PyMLR module includes the following main Python functions to facilitate the use of sklearn, XGBoost, CatBoost, LightGBM, statsmodels, shap, and optuna solvers for regression and classification: 
 ...
 - **lgbm_auto** - LightGBM LGBMRegressor (default) or LGBMClassifier (if optional keyword argument classify=True) with automatic cross-validated optimization of the hyperparameters
 - **lgbm** - LightGBM LGBMRegressor (default) or LGBMClassifier with user-specified hyperparameters
...
Hybrid functions for optimization of hyperparameters in 2-stage hybrid machine learning models, either combining two models in sequence, or using the same model with Recursive Feature Elimination, based on permutation importances of stage 1, include the following:
 ...
 - **adarfe_auto** - Recursive Feature Elimination using AdaBoost for feature selection (stage 1) and for classication or regression (stage 2)
...
- Hybrid models for optimization of hyperparameters in 2-stage hybrid machine learning models either combining two models in sequence or using the same model with Recursive Feature Elimination based on permutation importances of stage 1:
  ...
  - AdaBoost with Recursive Feature Elimination (**adarfe_auto**) is similar to the hybrid XGBoost-MLP method, except that stage 1 and 2 both use AdaBoost.
