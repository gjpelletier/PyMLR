...
The PyMLR module includes the following main Python functions to facilitate the use of sklearn, XGBoost, CatBoost, LightGBM, statsmodels, shap, and optuna solvers for linear and logistic regression: 
 ...
 - **xgb_auto** - XGBoost XGBRegressor (default) or XGBClassifier (with optional kwarg classify=True) with automatic cross-validated optimization of the hyperparameters
 - **xgb** - XGBRegressor (default) or XGBClassifier (with optional kwarg classify=True) with user-specified hyperparameters
